...
public partial class ProcessoPesagemApontamentoForm : Form
{
    ...
    public ProcessoPesagemApontamentoForm()
    {
        InitializeComponent();
    ...
    private async Task<bool> BloquearAcaoSemPermissaoAsync(string acao, string descricaoAcao)
    {
        ...
        await global::FugaPET_Dev.Tela.Comum.AcaoNegadaHelper.RegistrarAcaoNegadaSeguroAsync(
            AutorizacaoServico.ModuloProcesso, PermissoesSistema.Rotinas.LeituraProducao, acao, descricaoAcao, "ProcessoPesagemApontamentoForm");
    ...
    {
        int predictedWeight = productionDataGridView.Rows
            .Cast<DataGridViewRow>()
            .Where(row => !row.IsNewRow)
            .Sum(row => int.TryParse(GetCellValue(row, "productionQuantityColumn"), out int quantity) ? quantity : 0);

        int usedWeight = productionDataGridView.Rows
            .Cast<DataGridViewRow>()
            .Where(row => !row.IsNewRow)
            .Sum(row => int.TryParse(GetCellValue(row, "productionWeightColumn"), out int weight) ? weight : 0);

        boxesCounterLabel.Text = $"{predictedWeight:N0} kg";
        packagesCounterLabel.Text = $"{usedWeight:N0} kg";
        UpdateProductionGridFooter();
    }
    ...
    private async void LeituraManual_Click(object? sender, EventArgs e)
    {
        ...
        string currentWeight = GetCellValue(selectedRow, "productionWeightColumn");
        ...
        selectedRow.Cells["productionWeightColumn"].Value = normalizedWeight;
        ApplyProductionRowStyle(selectedRow, selectedRow.Index);
        ClearGridSelection(productionDataGridView);
        selectedRow.Selected = true;
        productionDataGridView.CurrentCell = selectedRow.Cells["productionWeightColumn"];
        UpdateProductionCounters();
    ...
    {
        if (!_isProductionStarted || e.RowIndex < 0 || e.ColumnIndex < 0)
        {
            if (!_isProductionStarted)
            {
                ClearGridSelection(productionDataGridView);
    ...
    private async void ProductionDataGridView_CellDoubleClick(object? sender, DataGridViewCellEventArgs e)
    {
        ...
        {
            string msg = await ErroUsuarioHelper.TratarAsync("IMPRESSAO_ETIQUETA_ERRO", ex, "ProcessoPesagemApontamentoForm",
    ...
    private void LoadMockData()
    {
        ...
        productionDataGridView.Rows.Add("58422", "TWIST STIX CARNE 24X50PCS", "24", "238");
        productionDataGridView.Rows.Add("58423", "TWIST STIX FRANGO 24X50PCS", "18", "182");
        productionDataGridView.Rows.Add("58424", "TWIST STIX BEEF 24X50PCS", "12", "124");
        productionDataGridView.Rows.Add("58425", "TWIST STIX BBQ 24X50PCS", "30", "301");
    }
