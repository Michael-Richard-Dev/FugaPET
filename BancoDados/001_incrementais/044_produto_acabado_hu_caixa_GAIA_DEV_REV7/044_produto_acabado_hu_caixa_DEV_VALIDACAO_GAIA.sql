...
-- FugaPET - Incremental 044 DEV - VALIDACAO REV5
...
BEGIN
 ...
 IF v_major <> 15 OR v_version_num < 150005 THEN
   RAISE EXCEPTION
     'CLASSIFICACAO_OFICIAL=VALIDACAO_044_DEV_REV5_BLOQUEADA_POSTGRESQL_INCOMPATIVEL major_atual=% versao_num_atual=% major_esperado=15 versao_minima=15.5',
     v_major,
     v_version_num;
 END IF;
 ...
 IF current_database()<>'fuga_jales_local_desenvolvimento' OR current_user<>'postgres' THEN
   RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=VALIDACAO_044_DEV_REV5_BLOQUEADA_AMBIENTE_OU_EXECUTOR';
 END IF;
...
BEGIN
    ...
    IF v_owner IS DISTINCT FROM v_expected.view_owner
       ...
       OR v_definition IS DISTINCT FROM v_expected.view_definition
       OR v_grants IS DISTINCT FROM v_expected.grants_signature
       OR v_columns IS DISTINCT FROM v_expected.columns_signature THEN
        RAISE EXCEPTION 'VIEW_METADATA_DIVERGENTE owner=%/% comentario_igual=% definicao_igual=% grants_iguais=% colunas_iguais=%',
    ...
    END IF;
    IF position('hu_palete_item' IN lower(v_definition))=0 THEN
        RAISE EXCEPTION 'VIEW_REGRA_DE_PALETE_NAO_PRESERVADA';
    END IF;
    SELECT
      ...
      INTO v_forbidden;
    IF v_forbidden<>0 THEN RAISE EXCEPTION 'VIEW_METADATA_ADICIONAL_NAO_PRESERVAVEL'; END IF;
END $gaia$;
...
DO $gaia$ BEGIN
 ...
 RAISE NOTICE 'CLASSIFICACAO_OFICIAL=VALIDACAO_044_DEV_REV5_CONCLUIDA_COM_SUCESSO';
...
\else
  \echo 'CLASSIFICACAO_OFICIAL=VALIDACAO_044_DEV_REV5_REPROVADA_RESIDUOS_GRANT_OU_SEQUENCE'
  \quit 3
\endif

\echo 'CLASSIFICACAO_OFICIAL=VALIDACAO_044_DEV_REV5_CONCLUIDA_COM_SUCESSO'
\echo 'OK - validacao 044 DEV REV5 concluida com ROLLBACK e zero residuos'
