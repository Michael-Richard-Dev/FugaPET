...
-- FugaPET - Incremental 044 DEV - REV5
-- Persistencia controlada de uma HU SAP para uma caixa individual de Produto Acabado.
-- Compatibilidade operacional REV5: PostgreSQL major 15, versao minima 15.5.
...
END
$gaia$;
    \echo 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_JA_APLICADA'
    \echo 'ESTADO_PACOTE=PACOTE_REV5_INTEGRALMENTE_APLICADO_NENHUMA_ALTERACAO_EXECUTADA'
\else
...
'FugaPET incremental 044 REV5: historico auditavel da leitura de peso. peso_lido preserva o valor bruto capturado pela balanca ou informado manualmente; peso_bruto, peso_liquido, peso_tara e codigo_usuario sao obrigatorios.';
...
BEGIN
    ...
    IF v_owner_after IS DISTINCT FROM v_owner_before
       ...
       OR v_definition_after IS DISTINCT FROM v_definition_before
       OR COALESCE(v_grants_after,ARRAY[]::text[]) IS DISTINCT FROM
          COALESCE(v_grants_before,ARRAY[]::text[]) THEN
        RAISE EXCEPTION
          'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_VIEW_NAO_PRESERVADA owner=%/% comentario_igual=% definicao_igual=% grants_iguais=%',
    ...
    PERFORM * FROM desenvolvimento.vw_hu_caixas_sem_palete LIMIT 1;
    RAISE NOTICE 'VIEW_VW_HU_CAIXAS_SEM_PALETE=PRESERVADA_E_ABERTA';
    RAISE NOTICE 'OBJETOS_PALETE_ALTERADOS=0';
END
$gaia$;
...
BEGIN
    ...
    IF v_role.rolsuper OR v_role.rolcreatedb OR v_role.rolcreaterole OR v_role.rolreplication OR v_role.rolbypassrls
       OR has_schema_privilege('fugapet_dev_app','desenvolvimento','CREATE') THEN
        RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_ROLE_INSEGURA';
    END IF;
    IF has_table_privilege('fugapet_dev_app','desenvolvimento.hu_caixa','INSERT')
       OR has_table_privilege('fugapet_dev_app','desenvolvimento.hu_caixa_pesagem','INSERT')
       OR has_table_privilege('fugapet_dev_app','desenvolvimento.hu_caixa','UPDATE')
       ...
       OR has_table_privilege('fugapet_dev_app','desenvolvimento.hu_caixa_integracao_sap','DELETE') THEN
        RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_PRIVILEGIOS_DIRETOS_INDEVIDOS';
    END IF;
    ...
    IF EXISTS (
      ...
      WHERE NOT has_column_privilege('fugapet_dev_app',format('desenvolvimento.%I',e.tabela),e.coluna,'INSERT')
        ...
        RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_GRANTS_COLUNA_AUSENTES';
    END IF;
    IF has_column_privilege('fugapet_dev_app','desenvolvimento.hu_caixa','codigo_hu_caixa','INSERT')
       ...
       OR has_column_privilege('fugapet_dev_app','desenvolvimento.hu_caixa_pesagem','situacao_hu_caixa_pesagem','INSERT') THEN
        RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_GRANTS_COLUNA_SISTEMICA';
    END IF;
    ...
    IF EXISTS (
      SELECT 1
       ...
       WHERE c.table_schema='desenvolvimento'
        ...
        RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_GRANTS_COLUNA_FORA_DA_LISTA';
    END IF;
    ...
    IF v_functions < 19 OR v_triggers<>5 OR v_indexes<>12 OR v_history_open<>0 OR v_public_execute<>0 OR v_app_execute<>13 THEN
        RAISE EXCEPTION 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_REPROVADA_POSTCHECK funcoes=% triggers=% indices=% historico_aberto=% public_execute=% app_execute=%/13',v_functions,v_triggers,v_indexes,v_history_open,v_public_execute,v_app_execute;
    END IF;
    RAISE NOTICE 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_APLICADA_COM_SUCESSO';
...
COMMIT;
\echo 'CLASSIFICACAO_OFICIAL=PROPOSTA_044_DEV_REV5_APLICADA_COM_SUCESSO'
\echo 'OK - incremental 044 DEV REV5 aplicado sem dados produtivos'
\endif
